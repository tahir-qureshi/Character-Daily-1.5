...
public class NotesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(NotesActivity.this);
...
//            Log.d("AllGrades1112", Gradedata);
        ...
        btnLesson.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                startActivity(new Intent(NotesActivity.this, ImageDownloading.class));
                finish();
            }
        });
        ...
        btnMore.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(NotesActivity.this, MoreOptionsActivity.class));
                finish();
            }
        });
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case android.R.id.home:
                ...
                Intent intent = new Intent(this, DailyLessonActivity.class);
                databaseReference = FirebaseDatabase.getInstance().getReference();
                postRef = FirebaseDatabase.getInstance().getReference().child("users").child(userId);
                DB.getInstance(NotesActivity.this).getScheduleLessonQuote(Globals.ClientID, 0, 1, Gradedata, new DBCallback<ScheduleLessonQuote>() {
                    @Override
                    public void success(final ScheduleLessonQuote o) {
                        ...
                        databaseReference.child("users").child(userId).child("notes").child(lessonID).setValue(feedback).addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@NonNull Task<Void> task) {
                                Toast.makeText(getApplicationContext(), "Inserted Notes", Toast.LENGTH_LONG).show();
                            }
                        });
